...
>
> 與 `LLM_MEMORY.md` 的分工：
> - `LLM_MEMORY.md` = **會衰減的**（現在做到哪、下一步、交接流水帳）。內容隨時間失效，〈交接日誌〉區塊超過 10 KB 時最舊條目會被壓縮封存（`AGENTS.md` §0〈防膨脹維護〉）。
...
| 程式碼慣例 | Python 3.x；每行程式後接中文尾註（逐行說明式）；函式開頭用 docstring；全專案 UTF-8 + 繁體中文 |

> 建置／測試／慣例等「技術脈絡」自 Playbook v10 起記在本節（不衰減層），不再放 `LLM_MEMORY.md`〈E〉——那份會被封存。
...
| 8 | 三份記憶檔按**衰減速度**分層（規則／會衰減／不衰減），不按主題（架構／測試／環境）分 | 主題會隨專案演化重新分類，衰減速度不會。同一檔裡混放「會過期的」與「不會過期的」，封存時必然連帶丟掉後者——這就是 LLM 換手失憶的主因 | 2026-09-03，Playbook v10 |
| 9 | 交接日誌**寫入時就分流**：日誌只記事件，「為什麼／試過不行／驗證了什麼」當場寫進本檔 | 原設計是「封存前才萃取」，那是治標。日誌會被封存離開視線，寫錯地方等於預約遺失；寫入時分流才是預防 | 2026-09-03，Playbook v10 |
